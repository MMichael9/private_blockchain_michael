...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if ( errors.length == 0 ) {
                ...
                block.hash = SHA256( JSON.stringify( block )).toString(); // compute hash
                self.chain.push( block ) 
                if( block ) {
                    self.height = self.height + 1;
                    resolve( 'Block pushed' )
                }
                else {
                    reject( 'It broke!' )
                }
                ...
                reject( 'Error found in the blockchain. Abort! ');
            }
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            self.chain.forEach( async( b ) => {
                // first check if hash is consistent, then check if prevblockhash matches
                if ( await b.validate() ) {

                    if ( ( b.height > 0 ) && ( b.previousBlockHash != self.chain[b.height - 1].hash ) ) {
                        errorLog.push( "block " + b.height + " is not valid" )
                    }
            ...
            resolve(errorLog);
            reject("Error occured when trying to validate the chain");
        });
    }
